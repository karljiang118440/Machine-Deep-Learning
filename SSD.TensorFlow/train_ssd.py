...
import os
import sys
#sys.path.append("F://Tensorflow//SSD.TensorFlow"）
...
#需要更改路径的设置
tf.app.flags.DEFINE_string(
    'data_dir', "/media/jcq/Doc/DL_data/VOC/dataset/tfrecords",
    'The directory where the dataset input data is stored.')
...
tf.app.flags.DEFINE_string(
    'model_dir', '/media/jcq/Doc/DL_data/VOC/logs',
    'The directory where the model will be stored.')

'''
# 更改logs路径
tf.app.flags.DEFINE_string(
    'model_dir', 'E:\Detections\SSD\logs',
    'The directory where the model will be stored.')
...
def validate_batch_size_for_multi_gpu(batch_size):
    ...
    if FLAGS.multi_gpu:
       ...
       # num_gpus = sum([1 for d in local_device_protos if d.device_type == 'CPU'])
        if not num_gpus:
